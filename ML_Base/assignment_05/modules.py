...
class Sequential(Module):
    ...
    def backward(self, input, gradOutput):
        ...
        result = gradOutput
        for i in range(1, n):
            result = self.modules[n - i].backward(self.output[n - i - 1], result)
        self.gradInput = self.modules[0].backward(input, result)
...
class Linear(Module):
    ...
    def accGradParameters(self, input, gradOutput):
        ...
        self.b = np.sum(gradOutput, axis=0)
...
class SoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = exp_output / np.expand_dims(np.sum(exp_output, axis=1), axis=1)
        return self.output
    
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        sofmax = self.output
        self.gradInput = (gradOutput - np.expand_dims(np.sum(gradOutput * sofmax, axis=1), axis=1)) * sofmax
        return self.gradInput
...
class LogSoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        batch_size = self.output.shape[0]
        exp_output = np.exp(self.output)
        self.output = np.log(exp_output / np.expand_dims(np.sum(exp_output, axis=1), axis=1))
        return self.output
    
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = gradOutput - np.exp(self.output) * np.expand_dims(np.sum(gradOutput, axis=1), axis=1)
        return self.gradInput
...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        ...
        if self.training:
            ...
            self.output = (input - input_mean) / np.sqrt(input_var + self.EPS)
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        batch_size, n_feats = gradOutput.shape
        ...
        #step 1
        divar = np.sum(gradOutput * xmu, axis=0)
        dxmu1 = gradOutput * ivar
        #step 2
        dsqrtvar = divar * (-1. / sqrtvar ** 2)
        #step 3
        dvar = dsqrtvar * 0.5 * (1. / np.sqrt(var + self.EPS))
        #step 4
        dsq = 1. / batch_size * np.ones((batch_size, n_feats)) * dvar
        #step 5
        dxmu2 = 2 * xmu * dsq
        #step 6
        dx1 = 1 * (dxmu1 + dxmu2)
        ...
        #step 7
        dx2 = 1. / batch_size * np.ones((batch_size, n_feats)) * dmu
        #step 8
        dx = dx1 + dx2
        self.gradInput = dx
...
class Dropout(Module):
    ...
    def updateOutput(self, input):
        ...
        if not self.training:
            ...
            dropout_input = 1. / (1 - self.p) * input * self.mask
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        if not self.training:
            ...
            dropout_output = 1. / (1 - self.p) * gradOutput * self.mask
            self.gradInput = dropout_output
...
class ELU(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = np.multiply(gradOutput, (input > 0) + self.alpha * np.exp(input) * (input <= 0))
        return self.gradInput
...
def adam_optimizer(variables, gradients, config, state):  
    ...
    for current_layer_vars, current_layer_grads in zip(variables, gradients): 
        for current_var, current_grad in zip(current_layer_vars, current_layer_grads):
            ...
            current_var = current_var - lr_t * var_first_moment / (np.sqrt(var_second_moment) + config['epsilon'])
